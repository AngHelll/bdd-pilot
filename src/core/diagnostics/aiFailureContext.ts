...
export interface BuildAiFailureContextOptions {
  ...
  /** Localized note when snapshot.provenance is rehydrated-trx (caller supplies i18n). */
  rehydratedFromTrxNote?: string;
}
...
export function buildAiFailureContext(
  ...
  const provenanceBlock = buildProvenanceBlock(snapshot, options);
...
function buildProvenanceBlock(
  ...
  if (snapshot.provenance !== "rehydrated-trx") {
    return "";
  }
  const file = snapshot.trxPath ? path.basename(snapshot.trxPath) : "TRX";
  const note =
    options?.rehydratedFromTrxNote ??
